...
/**
 * The only change made for flink L-BFGS implementation is the migration of the static variables to the
 * class "Statics" which is part of the parameters and the return.
 */
public class Mcsrch
{
	...
	public Statics mcsrch ( int n , double[] x , double f , double[] g , double[] s , int is0 , double[] stp , double ftol , double xtol , int maxfev , int[] info , int[] nfev , double[] wa,
								double gtol, double stpmin, double stpmax, Statics statics)
	{
		int infoc[] = new int[1], j = 0;
		double dg = 0, dgm = 0, dginit = 0, dgtest = 0, dgx[] = new double[1], dgxm[] = new double[1], dgy[] = new double[1], dgym[] = new double[1], finit = 0, ftest1 = 0, fm = 0, fx[] = new double[1], fxm[] = new double[1], fy[] = new double[1], fym[] = new double[1], p5 = 0, p66 = 0, stx[] = new double[1], sty[] = new double[1], stmin = 0, stmax = 0, width = 0, width1 = 0, xtrapf = 0;
		boolean brackt[] = new boolean[1], stage1 = false;

		infoc = statics.getInfoc();
		j = statics.getJ();
		dg = statics.getDg();
		dgm = statics.getDgm();
		dginit = statics.getDginit();
		dgtest = statics.getDgtest();
		dgx = statics.getDgx();
		dgxm = statics.getDgxm();
		dgy = statics.getDgy();
		dgym = statics.getDgym();
		finit = statics.getFinit();
		ftest1 = statics.getFtest1();
		fm = statics.getFm();
		fx = statics.getFx();
		fxm = statics.getFxm();
		fy = statics.getFy();
		...
		stx = statics.getStx();
		sty = statics.getSty();
		stmin = statics.getStmin();
		stmax = statics.getStmax();
		width = statics.getWidth();
		width1 = statics.getWidth1();
		xtrapf = statics.getXtrapf();
		brackt = statics.getBrackt();
		stage1 = statics.isStage1();
		...
		if ( info[0] != - 1 )
		{
			...
			if ( n <= 0 || stp[0] <= 0 || ftol < 0 || gtol < 0 || xtol < 0 || stpmin < 0 || stpmax < stpmin || maxfev <= 0 )
				return new Statics(infoc,  j , dg , dgm , dginit , dgtest ,dgx ,dgxm ,dgy ,dgym , finit , ftest1 , fm ,fx ,
					fxm ,fy ,fym , p5 , p66 ,stx ,sty , stmin , stmax , width , width1 , xtrapf ,brackt , stage1);
		...
		while ( true )
		{
			...
			if ( info[0] != 0 ) return new Statics(infoc,  j , dg , dgm , dginit , dgtest ,dgx ,dgxm ,dgy ,dgym , finit , ftest1 , fm ,fx ,
				fxm ,fy ,fym , p5 , p66 ,stx ,sty , stmin , stmax , width , width1 , xtrapf ,brackt , stage1);
