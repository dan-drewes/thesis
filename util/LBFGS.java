...
/**
 * For the Flink implementation some changes are made concerning input and output types
 * in order to integrate the program into the framework.
 */
public class LBFGS implements MapFunction<State, State>
{
	...
	*/
	...
	@Override
	public State map (State oldState) throws ExceptionWithIflag{
		int n = oldState.getN();
		int m = oldState.getM();
		double[] x = oldState.getX();
		double f = oldState.getF();
		double[] g = oldState.getG();
		boolean diagco = oldState.isDiagco();
		double[] diag = oldState.getDiag();
		int[] iprint = oldState.getIprint();
		double eps = oldState.getEps();
		double xtol = oldState.getXtol();
		int[] iflag = oldState.getIflag();

		double gtol = 0.9;
		double stpmin = 1e-20;
		double stpmax = 1e20;
		double[] solution_cache = null;

		double gnorm = 0, stp1 = 0, ftol = 0, stp[] = new double[1], ys = 0, yy = 0, sq = 0, yr = 0, beta = 0, xnorm = 0;
		int iter = 0, nfun = 0, point = 0, ispt = 0, iypt = 0, maxfev = 0, info[] = new int[1], bound = 0, npt = 0, cp = 0, i = 0, nfev[] = new int[1], inmc = 0, iycn = 0, iscn = 0;
		...
		gtol = oldState.getGtol();
		stpmin = oldState.getStpmin();
		stpmax = oldState.getStpmax();
		solution_cache = oldState.getSolution_cache();
		gnorm = oldState.getGnorm();
		stp1 = oldState.getStp1();
		ftol = oldState.getFtol();
		stp = oldState.getStp();
		ys = oldState.getYs();
		yy = oldState.getYy();
		sq = oldState.getSq();
		yr = oldState.getYr();
		beta = oldState.getBeta();
		xnorm = oldState.getXnorm();
		iter = oldState.getIter();
		nfun = oldState.getNfun();
		point = oldState.getPoint();
		ispt = oldState.getIspt();
		iypt = oldState.getIypt();
		maxfev = oldState.getMaxfev();
		info = oldState.getInfo();
		bound = oldState.getBound();
		npt = oldState.getNpt();
		cp = oldState.getCp();
		i = oldState.getI();
		nfev = oldState.getNfev();
		inmc = oldState.getInmc();
		iycn = oldState.getIycn();
		iscn = oldState.getIscn();
		finish = oldState.isFinish();
		w = oldState.getW();

		statics = oldState.getStatics();


		String history="Enter: "+iflag[0]+", "+maxfev+"\n";
		...
		history+="before: "+iflag[0]+", "+maxfev+"\n";
		while ( true )
		{
			history+="while: "+iflag[0]+", "+maxfev+"\n";
			if ( execute_entire_while_loop )
			{
				...
				if ( iter != 1 )
				{
					...
					{
						iflag[0]=2;
						System.out.println("diag shoud not be deployed manually");
						return null;
					}
				}
			}
			...
			error+="\nRest: "+(ispt + point * n)+", "+stp[0]+", "+ftol+", "+xtol+", "+maxfev+", "+info[0]+", "+nfev[0]+"\nDiag: ";
			for(int z=0;z<diag.length;z++){
				error+=""+diag[z]+",";
			}

			Mcsrch searcher = new Mcsrch();

			statics = searcher.mcsrch ( n , x , f , g , w , ispt + point * n , stp , ftol , xtol , maxfev , info , nfev , diag,
				gtol,stpmin,stpmax,statics);

			if ( info[0] == - 1 )
			{
				iflag[0]=1;
				//return new State(n, m, x, f, g, diagco , diag , iprint , eps , xtol , iflag);
				return new State(n, m, x, f, g, diagco , diag , iprint , eps , xtol , iflag,
				 gtol,stpmin,stpmax, solution_cache,gnorm,stp1,ftol,stp,ys,yy,sq,yr,beta,
				xnorm,iter,nfun,point,ispt,iypt,maxfev,info,bound,npt,cp,i, nfev,inmc,iycn,iscn,finish,w,statics);
			}
			...
			{
				iflag[0]=0;
				//return new State(n, m, x, f, g, diagco , diag , iprint , eps , xtol , iflag, maxfev);
				return new State(n, m, x, f, g, diagco , diag , iprint , eps , xtol , iflag,
					gtol,stpmin,stpmax, solution_cache,gnorm,stp1,ftol,stp,ys,yy,sq,yr,beta,
					xnorm,iter,nfun,point,ispt,iypt,maxfev,info,bound,npt,cp,i, nfev,inmc,iycn,iscn,finish,w,statics);
			}
